...
import static org.muyie.framework.config.MuyieConstants.*;
...
/**
 * Specific liquibase.integration.spring.SpringLiquibase that will update the database
 * asynchronously and close DataSource if necessary.
 * <p>
 ...
 * </ul>
 * But as this is a rather slow process, we use this asynchronous version to speed up our start-up
 * time:
 * <ul>
 * <li>On a recent MacBook Pro, start-up time is down from 14 seconds to 8 seconds</li>
 * <li>In production, this can help your application run on platforms like Heroku, where it must
 * start/restart very quickly</li>
 * </ul>
 */
public class AsyncSpringLiquibase extends DataSourceClosingSpringLiquibase {
  ...
  public void afterPropertiesSet() throws LiquibaseException {
    if (!env.acceptsProfiles(Profiles.of(SPRING_PROFILE_NO_LIQUIBASE))) {
      if (env
          .acceptsProfiles(Profiles.of(SPRING_PROFILE_DEVELOPMENT + "|" + SPRING_PROFILE_HEROKU))) {
