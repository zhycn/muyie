...
/**
 * <p>
 * MuyieMetricsEndpoint class.
 * </p>
 */
@SuppressWarnings("rawtypes")
@WebEndpoint(id = "muyie-metrics")
public class MuyieMetricsEndpoint {

  private final MeterRegistry meterRegistry;

  private final Logger logger = LoggerFactory.getLogger(MuyieMetricsEndpoint.class);
  ...
  /**
   * <p>
   * Constructor for MuyieMetricsEndpoint.
   * </p>
   *
   * @param meterRegistry a {@link io.micrometer.core.instrument.MeterRegistry} object.
   */
  public MuyieMetricsEndpoint(MeterRegistry meterRegistry) {
    this.meterRegistry = meterRegistry;
  }

  /**
   * GET /management/muyie-metrics
   * <p>
  ...
  private Map<String, Object> garbageCollectorMetrics() {
    ...
    Double classesLoaded =
        gauges.stream().map(Gauge::value).reduce((x, y) -> (x + y)).orElse((double) 0);
  ...
  private Map<String, Map<String, Number>> databaseMetrics() {
    ...
    timers.forEach(timer -> {
      ...
      ValueAtPercentile[] percentiles = timer.takeSnapshot().percentileValues();
      for (ValueAtPercentile percentile : percentiles) {
        resultsDatabase.get(key).put(String.valueOf(percentile.percentile()),
            percentile.value(TimeUnit.MILLISECONDS));
      }
    });
