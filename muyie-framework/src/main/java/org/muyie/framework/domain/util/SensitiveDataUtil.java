...
public class SensitiveDataUtil {
  ...
  /**
   * 对登陆帐号进行部分隐藏处理：如果是手机号，显示前3和后2位；如果是email，只显示用户名的前3位+*+@域名；如果都不是， 隐藏中间四位。 <br>
   *
   * @param logonId 待部分隐藏处理的登陆帐号，可能是手机号或email。
  ...
  public static String bankCardNoHide(final String bankCardNo, final boolean doValidate) {
    ...
    if (isBlank(bankCardNo)) return bankCardNo;
   ...
   *        <code>SensitiveDataUtil.PHONENO_DATA</code>、 <code>SensitiveDataUtil.EMAIL_DATA</code>或
