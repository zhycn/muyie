...
public class JwtTokenProvider {
  ...
  public JwtTokenProvider(MuyieProperties muyieProperties) {
  ...
  public void init() {
    byte[] keyBytes;
    String secret = muyieProperties.getSecurity().getAuthentication().getJwt().getSecret();
    if (!StringUtils.isEmpty(secret)) {
      ...
      keyBytes = Decoders.BASE64
          .decode(muyieProperties.getSecurity().getAuthentication().getJwt().getBase64Secret());
    }
    ...
    this.tokenValidityInMillisecondsForRememberMe = 1000 * muyieProperties.getSecurity()
