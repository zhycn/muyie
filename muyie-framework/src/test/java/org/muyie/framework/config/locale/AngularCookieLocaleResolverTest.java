package org.muyie.framework.config.locale;
...
import org.muyie.framework.config.locale.AngularCookieLocaleResolver;
...
public class AngularCookieLocaleResolverTest {
  ...
  public void testCookieWithQuotes() {
    ...
    assertThat(event.getMessage())
        .isEqualTo("Added cookie [" + DEFAULT_COOKIE_NAME + "=" + resolver.quote(value) + "]");
  ...
  public void testTraceLogLocaleAndTimeZone() {
    ...
    Event event = events.get(0);
    assertThat(event.getLevel()).isEqualTo("TRACE");
    assertThat(event.getMessage()).isEqualTo("Parsed cookie value [" + value + "] into locale '"
        + locale + "' " + "and time zone '" + zone.getID() + "'");
    assertThat(event.getThrown()).isNull();
  }
}
