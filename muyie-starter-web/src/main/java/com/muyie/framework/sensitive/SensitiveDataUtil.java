 ...
 * 示例：
 * <p>
 * 银行卡号： SensitiveDataUtil.bankCardNoHide("612526198319131434") = "612526********1434"
 * <br>
 * 身份证号： SensitiveDataUtil.idCardNoHide("362397201201012384") = "3****************4"
 * <br>
 * 邮箱： SensitiveDataUtil.emailHide("yaaachaa@163.com") = "yaa***@163.com"
 ...
 * <br>
 * 登录名： SensitiveDataUtil.logonIdHide("15087653459") = "150******59"
 * SensitiveDataUtil.logonIdHide("yaaachaa@163.com") = "yaa***@163.com"
 * <br>
 * 非上述固定格式敏感信息的屏蔽方法： SensitiveDataUtil.defaultHide("ttt") = "t*t"
 * <br>
 * 自定义屏蔽规则展示： SensitiveDataUtil.customizeHide("13568794561",3,4,4) = "135****4561"
 * <br>
 * 对起始标签和结尾标签中间的内容按指定的敏感数据类型进行部分隐藏: SensitiveDataUtil.filterHide(final String sourceStr,final String
 * tagBegin, final String tagEnd,final int sensitiveDataType ) sourceStr为：
 *
 * <pre>
 *  &lt;Party id=&quot;part_2&quot;&gt;
 *   &lt;FullName&gt;Jerry.jiang&lt;/FullName&gt;
...
class SensitiveDataUtil {
   ...
   * 大陆身份证号正则表达式
   */
  private static final String ID_CARD_REGEXP = "\\d{15}|\\d{18}|\\d{14}X|\\d{17}X";

  /**
   * 银行卡号正则表达式
   */
  private static final String BANK_CARD_REGEXP = "\\d{8,19}";

  /**
   * 手机或者固定电话正则表达式
   */
  private static final String MAINLAND_PHONE_TEL_REGEXP = "\\d{3,4}(-)?\\d{7,8}";
   ...
   * 手机或者固定电话匹配模式
   */
  private static final Pattern MAINLAND_PHONE_TEL_PATTERN =
    Pattern.compile(MAINLAND_PHONE_TEL_REGEXP);
  ...
  /**
   * 对登录帐号进行部分隐藏处理：如果是手机号，显示前3和后2位；如果是email，只显示用户名的前3位+*+@域名；如果都不是， 隐藏中间四位。 <br>
   *
   ...
   *
   * <ul>
   * <li>SensitiveDataUtil.nickHide("中华") = 中**华
   * <li>SensitiveDataUtil.nickHide("中华人") = 中**人
   * <li>SensitiveDataUtil.nickHide("中华人民") = 中**民
   * <li>SensitiveDataUtil.nickHide("china") = c**a
   * </ul>
   *
   * @param sensitiveData 昵称
   * @return 部分隐藏后的昵称
   */
  public static String nickHide(final String sensitiveData) {
    if (!needHide()) {
  ...
  /**
   * 对银行卡号进行部分隐藏处理， 如果银行卡位数大于等于16位只显示前6位和后4位，其他用*代替。<br> 如果银行卡位数小于16位，则使用缺省脱敏方式。<br>
  ...
  public static String emailHideSMS(final String email, final boolean doValidate) {
    ...
    if (dotIndex <= 7) {
      result.append(backStr, 0, dotIndex).append(".*");
    } else {
      result.append(backStr, 0, 7).append("*");
    }
   ...
   * <li>SensitiveDataUtil.emailHideSMS("ddddddd@netvigator.com") = ddd*@netviga*
   * <li>SensitiveDataUtil.emailHideSMS("d@netvigator.com") = d*@netviga*
   * <li>SensitiveDataUtil.emailHideSMS("ddddddd@outlook.com") = ddd*@outlook.*
   * </ul>
   *
   * @param email 待处理的Email
   * @return 如果hideFlag为true，返回《支付宝会员信息展示规范》中推荐的Email部分展示字符串；否则返回原数据。
   ...
   *
   * <pre>
   * <code>tagBegin:&lt;GovtID&gt;</code> <code>tagEnd: &lt;/GovtID&gt;</code>
   * <code>sensitiveInfoType: SensitiveDataUtil.IDCARDNO_DATA</code> </pre>
   * <p>
   * 方法返回值:
   *
   * <pre>
  ...
  public static String filterHide(final String sourceStr, final String tagBegin,
    ...
    while (begin != -1 && end != -1) {
      // 说明存在要求的节点
      target.append(tmp.toString().toCharArray(), 0, begin + tagBegin.length());
      // 加入过滤字符
      String coverReplace = "";
   ...
   * @return 屏蔽后的数据; 如果hideFlag为false，返回原数据。
   */
  ...
  /**
   * 通过正则表达式"\\d{15}|\\d{18}|\\d{14}X|\\d{17}X"来判断是否是合法的大陆身份证号。
   *
   * @param idCardNo 大陆身份证号
   * @return 指示入参是否大陆身份证号的布尔值
   */
  ...
  /**
   * 通过正则表达式"\\d{3,4}[-]?\\d{7,8}"来判断是否是合法的电话号码。 只判断是否符合国内电话号码
   *
   * @param phoneOrTelNo 手机号或电话号码
